...
- In `著者（年）` citations for X posts, write the author as `@account_id（年）`, using the account ID from the post URL rather than the display name. Use the same `@account_id` in the bibliography entry and all in-text citations.
- In `著者（年）` citations for social media other than X, GitHub, and similar web-service accounts, use the account notation displayed on that service as `著者`. Preserve the service's spelling and capitalization instead of converting it to a personal-name or organization-name style.
...
- X posts use `@account_id` rather than the display name consistently in author-year citations and bibliography entries, and their URLs use `https://x.com/{account_id}/status/{post_id}`.
- Social media other than X, GitHub, and similar web-service accounts use the notation displayed on the service consistently in both author-year citations and bibliography entries.
